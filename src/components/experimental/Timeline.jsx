...
const Timeline = () => {
  ...
  return (
    <section ref={ref} className="py-32 bg-midnight text-champagne relative px-4 md:px-0">
      ...
      <div className="max-w-5xl mx-auto relative">
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div key={idx} className={`relative flex flex-col md:flex-row items-center justify-between mb-24 ${isEven ? 'md:flex-row-reverse' : ''}`}>
              
              {/* Dot */}
              <div className="absolute left-[17px] md:left-[calc(50%-7px)] w-4 h-4 bg-[#141a23] border-[3px] border-gold rounded-full z-10 shadow-[0_0_15px_rgba(212,175,55,0.4)]" />
              ...
              <motion.div 
                ...
                className={`w-full pl-16 md:pl-0 md:w-[45%] group cursor-none hover-target`}
              >
                <div className="ios-card p-4 mb-6">
                  <div className="relative overflow-hidden rounded-[24px]">
                    ...
                    <img src={step.image} alt={step.title} className="w-full h-64 object-cover opacity-80 group-hover:opacity-100 group-hover:scale-105 transition-all duration-1000" />
                  </div>
                </div>
                <div className={`px-4 ${isEven ? 'md:text-right' : 'md:text-left'}`}>
                  <span className="text-gold font-sans uppercase tracking-[0.3em] text-[10px] mb-2 block">Phase 0{idx + 1}</span>
                  <h3 className="font-serif text-3xl mb-3 text-champagne">{step.title}</h3>
